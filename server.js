...
async function initMySQL() {
  ...
  await pool.execute(`CREATE TABLE IF NOT EXISTS sessions (token VARCHAR(64) PRIMARY KEY, user_id VARCHAR(36) NOT NULL, expires_at VARCHAR(50) NOT NULL)`);
  await pool.execute(`CREATE TABLE IF NOT EXISTS rooms (id VARCHAR(36) PRIMARY KEY, name VARCHAR(50) NOT NULL, building VARCHAR(50) NOT NULL, capacity INT NOT NULL)`);
  await pool.execute(`CREATE TABLE IF NOT EXISTS bookings (id VARCHAR(36) PRIMARY KEY, user_id VARCHAR(36) NOT NULL, room_id VARCHAR(36) NOT NULL, day VARCHAR(20) NOT NULL, slot VARCHAR(30) NOT NULL, purpose VARCHAR(255) NOT NULL, status VARCHAR(20) DEFAULT 'PENDING', created_at VARCHAR(50) NOT NULL)`);
  await pool.execute(`CREATE TABLE IF NOT EXISTS faculty (id VARCHAR(36) PRIMARY KEY, name VARCHAR(255) NOT NULL, department VARCHAR(50) NOT NULL, user_id VARCHAR(36), is_scheduling_only TINYINT(1) DEFAULT 0)`);
  await pool.execute(`CREATE TABLE IF NOT EXISTS schedule (id VARCHAR(36) PRIMARY KEY, day VARCHAR(20) NOT NULL, slot VARCHAR(30) NOT NULL, room_id VARCHAR(36) NOT NULL, faculty_id VARCHAR(36) NOT NULL, subject VARCHAR(100) NOT NULL, section VARCHAR(20) NOT NULL)`);
}

function uid() { return crypto.randomUUID(); }
function hashPwd(pwd, salt) { return crypto.scryptSync(pwd, salt, 64).toString("hex"); }

async function dbGet(sql, params = []) { const [rows] = await pool.execute(sql, params); return rows[0] || null; }
async function dbAll(sql, params = []) { const [rows] = await pool.execute(sql, params); return rows; }
async function dbRun(sql, params = []) { const [result] = await pool.execute(sql, params); return { changes: result.affectedRows }; }
...
async function authenticate(req) {
  ...
  const sess = await dbGet("SELECT s.token, u.id, u.identifier, u.name, u.role, u.is_temp FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = ? AND s.expires_at > ?", [auth.slice(7), new Date().toISOString()]);
...
async function getAllData() {
  return {
    rooms: await dbAll("SELECT * FROM rooms ORDER BY building, name"),
    faculty: await dbAll("SELECT * FROM faculty ORDER BY name"),
    schedule: await dbAll("SELECT s.id, s.day, s.slot, s.room_id AS roomId, s.faculty_id AS facultyId, s.subject, s.section, r.name AS roomName, r.building, f.name AS facultyName, f.department FROM schedule s JOIN rooms r ON r.id = s.room_id JOIN faculty f ON f.id = s.faculty_id ORDER BY s.day, s.slot"),
    days: DAYS,
    slots: SLOTS
  };
}
...
async function handleApi(req, res, path_) {
  ...
  if (m === "POST" && path_ === "/api/login") {
    ...
    const user = await dbGet("SELECT * FROM users WHERE identifier = ?", [identifier.trim()]);
    ...
    await dbRun("INSERT INTO sessions VALUES (?, ?, ?)", [token, user.id, new Date(Date.now() + 12 * 3600 * 1000).toISOString()]);
  ...
  if (m === "POST" && path_ === "/api/logout") {
    ...
    if (s) await dbRun("DELETE FROM sessions WHERE token = ?", [s.token]);
  ...
  if (m === "POST" && path_ === "/api/change-password") {
    ...
    await dbRun("UPDATE users SET salt = ?, password_hash = ?, is_temp = 0 WHERE id = ?", [salt, hashPwd(new_password, salt), s.id]);
    return json(res, 200, { ok: true });
  }
  ...
  if (m === "POST" && path_ === "/api/bookings") {
    ...
    const booked = await dbGet("SELECT id FROM bookings WHERE room_id = ? AND day = ? AND slot = ? AND status = ?", [roomId, day, slot, "APPROVED"]);
    if (booked) return json(res, 400, { error: "Room already booked" });
    const reserved = await dbGet("SELECT id FROM schedule WHERE room_id = ? AND day = ? AND slot = ?", [roomId, day, slot]);
    if (reserved) return json(res, 400, { error: "Room reserved" });
    await dbRun("INSERT INTO bookings (id, user_id, room_id, day, slot, purpose, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [uid(), s.id, roomId, day, slot, purpose.trim().toUpperCase(), "PENDING", new Date().toISOString()]);
    return json(res, 200, { ok: true });
  }
  ...
  if (m === "GET" && path_ === "/api/bookings") {
    const s = await requireAuth(req, res);
    if (!s) return;
    return json(res, 200, { bookings: await dbAll("SELECT b.id, b.day, b.slot, b.purpose, b.status, b.created_at, r.name AS roomName, r.building FROM bookings b JOIN rooms r ON r.id = b.room_id WHERE b.user_id = ? ORDER BY b.day, b.slot", [s.id]) });
  }

  if (m === "DELETE" && path_.startsWith("/api/bookings/")) {
    const s = await requireAuth(req, res);
    if (!s) return;
    await dbRun("DELETE FROM bookings WHERE id = ?", [path_.split("/").pop()]);
    return json(res, 200, { ok: true });
  }
  ...
  if (m === "GET" && path_ === "/api/admin/users") {
    ...
    if (!s) return;
    return json(res, 200, { users: await dbAll("SELECT id, identifier, name, role, is_temp, created_at FROM users ORDER BY role, name") });
  }
  ...
  if (m === "DELETE" && path_.startsWith("/api/admin/users/")) {
    ...
    await dbRun("DELETE FROM users WHERE id = ?", [uid2]);
    return json(res, 200, await getAllData());
  }
  ...
  if (m === "POST" && path_ === "/api/admin/reset-password") {
    ...
    const salt = crypto.randomBytes(16).toString("hex");
    await dbRun("UPDATE users SET salt = ?, password_hash = ?, is_temp = 1 WHERE id = ?", [salt, hashPwd(pwd, salt), user_id]);
  ...
  if (m === "POST" && path_ === "/api/admin/rooms") {
    ...
    if (await dbGet("SELECT id FROM rooms WHERE name = ? AND building = ?", [n, b])) return json(res, 400, { error: "Room exists" });
    await dbRun("INSERT INTO rooms VALUES (?, ?, ?, ?)", [uid(), n, b, Number(capacity)]);
    return json(res, 200, await getAllData());
  }
  ...
  if (m === "DELETE" && path_.startsWith("/api/admin/rooms/")) {
    const s = await requireAdmin(req, res);
    if (!s) return;
    await dbRun("DELETE FROM rooms WHERE id = ?", [path_.split("/").pop()]);
    return json(res, 200, await getAllData());
  }
  ...
  if (m === "POST" && path_ === "/api/admin/faculty") {
    ...
    if (await dbGet("SELECT id FROM users WHERE identifier = ?", [id])) return json(res, 400, { error: "ID exists" });
    const salt = crypto.randomBytes(16).toString("hex");
    const fid = uid(), uid2 = uid();
    await dbRun("INSERT INTO faculty (id, name, department, user_id, is_scheduling_only) VALUES (?, ?, ?, ?, 0)", [fid, nm, "FACULTY", uid2]);
    await dbRun("INSERT INTO users (id, identifier, name, role, salt, password_hash, is_temp, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [uid2, id, nm, "FACULTY", salt, hashPwd(password, salt), 1, new Date().toISOString()]);
    return json(res, 200, await getAllData());
  }
  ...
  if (m === "DELETE" && path_.startsWith("/api/admin/faculty/")) {
    const s = await requireAdmin(req, res);
    if (!s) return;
    await dbRun("DELETE FROM faculty WHERE id = ?", [path_.split("/").pop()]);
    return json(res, 200, await getAllData());
  }
  ...
  if (m === "POST" && path_ === "/api/admin/students") {
    ...
    if (await dbGet("SELECT id FROM users WHERE identifier = ?", [id])) return json(res, 400, { error: "ID exists" });
    const salt = crypto.randomBytes(16).toString("hex");
    await dbRun("INSERT INTO users (id, identifier, name, role, salt, password_hash, is_temp, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [uid(), id, nm, "STUDENT", salt, hashPwd(password, salt), 1, new Date().toISOString()]);
    return json(res, 200, { ok: true });
  }
  ...
  if (m === "POST" && path_ === "/api/admin/schedule") {
    ...
    await dbRun("INSERT INTO schedule VALUES (?, ?, ?, ?, ?, ?, ?)", [uid(), day, slot, roomId, facultyId, subject.trim().toUpperCase(), section.trim().toUpperCase()]);
    return json(res, 200, await getAllData());
  }
  ...
  if (m === "DELETE" && path_.startsWith("/api/admin/schedule/")) {
    const s = await requireAdmin(req, res);
    if (!s) return;
    await dbRun("DELETE FROM schedule WHERE id = ?", [path_.split("/").pop()]);
    return json(res, 200, await getAllData());
  }
  ...
  if (m === "GET" && path_ === "/api/admin/bookings") {
    const s = await requireAdmin(req, res);
    if (!s) return;
    return json(res, 200, { bookings: await dbAll("SELECT b.id, b.day, b.slot, b.purpose, b.status, b.created_at, r.name AS roomName, r.building, u.name AS userName, u.identifier FROM bookings b JOIN rooms r ON r.id = b.room_id JOIN users u ON u.id = b.user_id ORDER BY b.created_at DESC") });
  }
  ...
  if (m === "POST" && path_.startsWith("/api/admin/bookings/")) {
    ...
    await dbRun("UPDATE bookings SET status = ? WHERE id = ?", [status, bid]);
    return json(res, 200, { ok: true });
  }
...
server.listen(PORT, async () => {
  ...
  await seedData();
  console.log(`NIST RM → http://localhost:${PORT}`);
});
