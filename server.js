...
async function handleApi(req, res, path_) {
  ...
  if (m==="GET" && path_==="/api/bookings") { const s = requireAuth(req, res); if (!s) return; const myId = s.id; return json(res, 200, { bookings: await dbAll("SELECT b.id, b.day, b.slot, b.purpose, b.status, b.created_at, r.name AS roomName, r.building FROM bookings b JOIN rooms r ON r.id = b.room_id WHERE b.user_id = ? ORDER BY b.day, b.slot", [myId]) }); }
