...
async function handleApi(req, res, path_) {
  ...
  if (m==="POST" && path_==="/api/bookings") { const s = requireAuth(req, res); if (!s) return; const { roomId, day, slot, purpose } = await readBody(req); if (!roomId || !day || !slot || !purpose) return json(res, 400, { error: "All fields required" }); if (!SLOTS.includes(slot)) return json(res, 400, { error: "Invalid slot" }); if (await dbGet("SELECT id FROM bookings WHERE room_id=? AND day=? AND slot=? AND status='APPROVED'", [roomId, day, slot])) return json(res, 400, { error: "Room already booked" }); if (await dbGet("SELECT id FROM schedule WHERE room_id=? AND day=? AND slot=?", [roomId, day, slot])) return json(res, 400, { error: "Room reserved" }); await dbRun("INSERT INTO bookings (id,user_id,room_id,day,slot,purpose,status,created_at) VALUES (?,?,?,?,?,?,?,?)", [uid(), s.id, roomId, day, slot, purpose.trim().toUpperCase(), "PENDING", new Date().toISOString()]); return json(res, 200, { ok: true }); }
  if (m==="GET" && path_==="/api/bookings") { const s = requireAuth(req, res); if (!s) return; return json(res, 200, { bookings: await dbAll("SELECT b.id,b.day,b.slot,b.purpose,b.status,b.created_at,r.name AS roomName,r.building FROM bookings b JOIN rooms r ON r.id=b.room_id WHERE b.user_id=? OR b.status='APPROVED' ORDER BY b.day,b.slot", [s.id]) }); }
  if (m==="DELETE" && path_.startsWith("/api/bookings/")) { const s = requireAuth(req, res); if (!s) return; await dbRun("DELETE FROM bookings WHERE id=? AND user_id=?", [path_.split("/").pop(), s.id]); return json(res, 200, { ok: true }); }
